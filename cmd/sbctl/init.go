...
func cmdInit() {
	...
	// 检查是否有有意义的节点配置
	if config.HasMeaningfulConfig(config.DefaultConfigPath) {
		cfg, _ = config.ReadConfig(config.DefaultConfigPath)
		nodeCount := len(cfg.Inbounds) + len(cfg.Endpoints)
